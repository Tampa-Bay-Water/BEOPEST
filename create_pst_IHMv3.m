function create_pst(d_root,runname,wt_col,flag_parmtran)
% Script to create PPEST *.pst (version2)

if nargin < 1 || isempty(d_root)
  d_root = 'F:\IHM\beopest';
end
if nargin < 2, runname = 'bp_013'; end
if nargin < 3, wt_col = 1; end
if nargin < 4, flag_parmtran = true; end

%% path and files for creation of parameters data
f_param = fullfile(d_root,'parameters_3.xls'); % original:'parameters_2.xls'
f_weight = fullfile(d_root,'ObsWeightSchemes_2.xls'); %original:ObsWeightSchemes_2.xls
d_graphicpest = fullfile(d_root,'Graphics_Pest');
f_intb = fullfile(d_root,'Current0','INTB.mdb');
...
conn = database('','','',...
  ...
  ['jdbc:odbc:Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=',f_intb,';']);
setdbprefs('DataReturnFormat','table');
scn_props = fetch(conn,'select * from Scenario where Name=''PEST_Run''');
...
if all(cellfun(@(y) isnan(y),parent_par(:,wt_col)))
    parent_par = [];
else
...
%     system('SqlLocalDB.exe stop v11.0');
    system('SqlLocalDB.exe start v11.0');

    sv = '(localdb)\v11.0';
    dv = 'SQL Server Native Client 11.0';
    % dv = 'Microsoft OLE DB Provider for SQL Server';
    fn = fullfile(d_root,'Current0','INTB_output.mdf');
    [~,dbn] = fileparts(fn);
    conn = database('','','','sun.jdbc.odbc.JdbcOdbcDriver',...
        ['jdbc:odbc:Driver=' dv ';Server=' sv ';AttachDbFileName=' fn ...
        ';Database=' dbn ';Trusted_Connection=Yes']);
...
%         csv{1,1}(temp_i),csv{1,2}(temp_i),'UniformOutput',false);
    ...
    close(conn);
    system('SqlLocalDB.exe stop v11.0');
    system('SqlLocalDB.exe delete v11.0');
    ...
    t_potsurf = create_potsurfacetarget(d_root,data_sdate,data_edate);
    ...
    ins_line{i_insfile} = cellfun(@(x) sprintf('L1 !POT_%s!',x),t_potsurf.PSWellID,...
        'UniformOutput',false);
%{
    fid = fopen(fullfile(d_root,runname,char(f_inst{i_insfile})),'wt');
    fprintf(fid,'%s\n','pif @');
